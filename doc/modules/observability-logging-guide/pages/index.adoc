...
In this guide, we use Grafana Loki as the centralized logging backend to collect and store application logs.

Grafana Loki is designed for efficient and scalable log storage, particularly in modern, cloud-native environments. Unlike traditional full-text search systems, Loki indexes only selected metadata fields, called labels—such as `app`, `env`, or `service`—while the full log messages are stored separately without full indexing.

This architecture significantly reduces memory usage and storage requirements, offering a cost-effective solution for centralized logging. Loki integrates seamlessly with other components of the observability stack, such as Grafana for visualization, Prometheus for metrics, and Tempo for distributed tracing.
...
While OpenTelemetry also provides APIs and SDKs to instrument applications directly, we use the **Spring Boot Actuator** to expose structured logs in this guide.

Spring Boot Actuator provides a unified way to expose not only logs, but also health checks, metrics, and other operational endpoints. Additionally, it now supports exporting distributed tracing data, which we will explore in more detail in the next part of this guide.
